-- AQUASTROKE — Migration 001: Core Schema
...
CREATE TABLE IF NOT EXISTS seasons (
  ...
  current_phase TEXT NOT NULL DEFAULT 'GPP' CHECK (current_phase IN ('GPP','SPP1','SPP2','COMP','TAPER','CHAMP')),
  start_date DATE NOT NULL DEFAULT CURRENT_DATE,
  volume_gpp INT, volume_spp1 INT, volume_spp2 INT, volume_comp INT, volume_taper INT,
...
CREATE TABLE IF NOT EXISTS athletes (
  ...
  dob DATE,
  event TEXT NOT NULL,
  category TEXT NOT NULL DEFAULT 'Middle' CHECK (category IN ('Sprint','Middle','Distance')),
  css_velocity NUMERIC(6,3),
  notes TEXT,
  is_active BOOLEAN NOT NULL DEFAULT TRUE,
  target_t1 NUMERIC(8,2), target_t2 NUMERIC(8,2), target_t3 NUMERIC(8,2),
...
CREATE TABLE IF NOT EXISTS trial_results (
  ...
  context TEXT NOT NULL DEFAULT 'Normal' CHECK (context IN ('Normal','Excellent','Exam','Illness','Injury','Equipment')),
...
DO $$ DECLARE t TEXT; BEGIN
  FOREACH t IN ARRAY ARRAY['academies','coaches','seasons','athletes','trial_results'] LOOP
    ...
    EXECUTE 'CREATE TRIGGER set_updated_at BEFORE UPDATE ON '||t||' FOR EACH ROW EXECUTE FUNCTION update_updated_at()';
  END LOOP;
END $$;
